...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    all_score = 0
    score, sign = score ** 3, 1
    while score:
        all_score = all_score + score % 10 * sign
        sign, score = sign * (-1), score // 10
    all_score = abs(all_score) + 1
    return all_score    
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    all_score = 3 ** (player_score + opponent_score)
    final_digit = all_score % 10
    first_digit = False
    while all_score:
        curr_digit = all_score % 10
        if curr_digit == final_digit and all_score // 10 == 0:
            first_digit = True
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    get_score0, get_score1 = 0, 0
    if feral_hogs == True:
        while score0 < goal and score1 < goal:
            if who == 0:
                num_rolls = strategy0(score0, score1)
                if abs(num_rolls - get_score0) == 2:
                    score0 = score0 + 3
                get_score0 = take_turn(num_rolls, score1, dice)
                score0 = score0 + get_score0
            else:
                num_rolls = strategy1(score1, score0)
                if abs(num_rolls - get_score1) == 2:
                    score1 = score1 + 3
                get_score1 = take_turn(num_rolls, score0, dice)
                score1 = score1 + get_score1
            who = other(who)
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    # BEGIN PROBLEM 7
    def hightest(score0, score1,prev_high = prev_high, prev_score = prev_score):
        if who == 0:
            score = score0
        else:
            score = score1
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    each_average = make_averaged(roll_dice)
    num = 10
    prev_num = 0
    prev_aver = 0
    while num >= 1:
        average = each_average(num,dice)
        if average > prev_aver:
            prev_aver = average
            prev_num = num
        num = num - 1
    return prev_num
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    if is_swap(new_score, opponent_score) and new_score < opponent_score:
        return 0
    elif free_bacon(opponent_score) >= margin and not(is_swap(new_score, opponent_score) and new_score > opponent_score):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
